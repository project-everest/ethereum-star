...
val sel: #key:eqtype -> #value:Type -> m:mapping key value -> key
         -> ST value (requires (fun h -> contains h m)) (ensures (fun h x h' -> h == h'))
let sel #key #value m k = FStar.Map.sel (read m) k

val upd: #key:eqtype -> #value:Type -> m:mapping key value -> key -> value
         -> ST unit (requires (fun h -> contains h m))

...
let lockMyEther () : unit =
  update (state.owner) (msg.sender);
    update_map (state.fees) (state.owner) (0)
...
let main () : unit =
  let fee : uint = div (msg.value) (200) in
    update_map (state.fees) (state.owner) (add (lookup (state.fees) (state.owner)) (fee));
    let deposit : uint = sub (msg.value) (fee) in
    update_map (state.user) (msg.sender) ({lookup (state.user) (msg.sender) with balance = add (balance (lookup (state.user) (msg.sender))) (deposit)});
    if (gt ((lookup (state.user) (msg.sender)).expiration_block) (0)) then
    ()
  else
    update_map (state.user) (msg.sender) ({lookup (state.user) (msg.sender) with expiration_block = 0})
...
let withdraw () : unit =
  if ((eq ((lookup (state.user) (msg.sender)).trickle) (true)) && (gt ((lookup (state.user) (msg.sender)).expiration_block) (block.number))) then
    let balance_per_block : uint = div (balance (lookup (state.user) (msg.sender))) (sub ((lookup (state.user) (msg.sender)).expiration_block) ((lookup (state.user) (msg.sender)).trickle_block)) in
        ...
        update_map (state.user) (msg.sender) ({lookup (state.user) (msg.sender) with balance = sub (balance (lookup (state.user) (msg.sender))) (trickle_balance)});
        update_map (state.user) (msg.sender) ({lookup (state.user) (msg.sender) with trickle_block = block.number})
  else
    if (gt ((lookup (state.user) (msg.sender)).expiration_block) (block.number)) then
      ...
      send (msg.sender) (balance (lookup (state.user) (msg.sender)));
            update_map (state.user) (msg.sender) ({lookup (state.user) (msg.sender) with balance = 0});
            update_map (state.user) (msg.sender) ({lookup (state.user) (msg.sender) with expiration_block = 0})
...
let setExpirationBlock (_blocknumber:uint) : unit =
  if (gt ((lookup (state.user) (msg.sender)).expiration_block) (0)) then
    ...
    if (gt (_blocknumber) (add (block.number) (9000000))) then
      ...
      update_map (state.user) (msg.sender) ({lookup (state.user) (msg.sender) with expiration_block = _blocknumber})

let setTrickleOn () : unit =
  update_map (state.user) (msg.sender) ({lookup (state.user) (msg.sender) with trickle = true});
    update_map (state.user) (msg.sender) ({lookup (state.user) (msg.sender) with trickle_block = block.number})

let setTrickleOff () : unit =
  update_map (state.user) (msg.sender) ({lookup (state.user) (msg.sender) with trickle = false})

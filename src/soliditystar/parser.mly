...
%token RETURNS EVENT ENUM
%token MODIFIER CONTRACT
%token STRUCT MAPPING MAPSTO
%token NULL INTERNAL EXTERNAL
%token PRIVATE PUBLIC CONSTANT
...
main:
| EOL* a = list(source_element) EOF {a}

source_element:
| CONTRACT x=IDENTIFIER EOL* LC EOL* l=list(decl) RC EOL* {Contract(x, l), ($startpos,$endpos)}

decl:
| t = optional_semi(statevar) {t}
| EVENT e = optional_semi(event) {e}
| STRUCT x=IDENTIFIER EOL* LC EOL* l=list(optional_semi(typedvar)) RC EOL* {Typedef(x, l)}
| ENUM t=IDENTIFIER LC EOL* l=enum RC EOL* {Enum(t,l)}
| MODIFIER x = IDENTIFIER LP args = separated_list(COMMA, typedvar) RP EOL* LC EOL* body=function_body RC EOL*
  {Modifier(Some x, args, body)}
| FUNCTION n = option(IDENTIFIER) LP args = separated_list(COMMA, typedvar) RP EOL* fq=list(fquals_eof) LC EOL* body=function_body RC EOL*
  {Method(fq, (n, args, body))}

statevar:
| t = typev tvqual x=IDENTIFIER e = option(optstateval) {Statevar(t, x, e)}

optstateval:
| ASSIGN e = full_commaless_expression(base_expression) {e}

enum:
| EOL* {[]}
| t=IDENTIFIER EOL* {[t]}
| t=IDENTIFIER COMMA r=enum {t::r}

fquals_eof:
| f=fquals EOL* {f}

fquals:
| x=IDENTIFIER a = modargs {Modified(x, a)}
| INTERNAL {Internal}
| EXTERNAL {External}
| PUBLIC {Public}
| PRIVATE {Private}
| CONSTANT {Constant}
| RETURNS LP t=typev x=option(IDENTIFIER) RP {Returns(t,x)}

modargs:
| {None}
| LP el=qargs RP {Some el}

qargs:
| v = separated_list(COMMA, full_commaless_expression(base_expression)) {v}

function_body:
| body=list(st_statement) {Globals.re:=false; body}

event:
| x=IDENTIFIER LP l=separated_list(COMMA, typedvar) RP {Event(x, l)}

typedvar:
| t=typev x=IDENTIFIER {(t,x)}
...
typev:
| MAPPING LP x=IDENTIFIER MAPSTO t=typev RP {Mapping(x,t)}
| t=typev LB RB {Array(t)}
| x=IDENTIFIER {Named(x)}
...
| v = var_decl(full_commaless_expression)   {let t,v = v in (Declaration(t,v),($startpos,$endpos))}
| RETURN e=expression? {(Return(e),($startpos,$endpos))}
| THROW {(Throw,($startpos,$endpos))}
...
var_decl(X):
| t = IDENTIFIER v = separated_nonempty_list(COMMA, var_declaration(X)) {(t,v)}
| VAR v = separated_nonempty_list(COMMA, var_declaration(X)) {("int",v)}

for_init:
| v = var_decl(inless_expression) {let t,v = v in (Declaration(t,v),($startpos,$endpos))}
...
lhs_expression(X):
| NEW e = new_expression {(let e,o = e in New(e, o),($startpos,$endpos))}
...
| FUNCTION n = IDENTIFIER? LP args = separated_list(COMMA, typedvar) RP EOL* LC b=function_body RC EOL
...
property:
| property_name COLON full_commaless_expression(base_expression) {(($1,$3),($startpos,$endpos))}
